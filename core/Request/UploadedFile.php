...
use Booking\Exception\UploadedFileAlreadyMovedException;
...
class UploadedFile implements UploadedFileInterface
{
    ...
    public function getStream() : StreamInterface
    {
        if ($this->error !== UPLOAD_ERR_OK) {
            throw UploadedFileErrorException::dueToStreamUploadError(
     ...
     * @throws UploadedFileErrorException if the upload was not successful.
     * @throws InvalidArgumentException if the $path specified is invalid.
     * @throws UploadedFileErrorException on any error during the
     *     move operation, or on the second or subsequent call to the method.
     */
    public function moveTo($targetPath) : void
    {
        if ($this->moved) {
            throw new UploadedFileAlreadyMovedException('Cannot move file; already moved!');
        }
        ...
        if (! is_dir($targetDirectory) || ! is_writable($targetDirectory)) {
            throw UploadedFileErrorException::dueToUnwritableTarget($targetDirectory);
        }
        ...
        switch (true) {
            ...
            default:
                ...
                if (false === move_uploaded_file($this->file, $targetPath)) {
                    throw UploadedFileErrorException::forUnmovableFile();
                }
                break;
        }
    ...
    private function writeFile(string $path) : void
    {
        ...
        if (false === $handle) {
            throw UploadedFileErrorException::dueToUnwritablePath();
        }
